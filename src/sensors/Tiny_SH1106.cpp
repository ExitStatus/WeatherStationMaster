...
void Tiny_SH1106::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
Tiny_SH1106::Tiny_SH1106(int8_t SID, int8_t SCLK, int8_t DC, int8_t RST, int8_t CS) : Adafruit_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT) {
  cs = CS;
  rst = RST;
  dc = DC;
  sclk = SCLK;
...
Tiny_SH1106::Tiny_SH1106(int8_t reset) :
...
void Tiny_SH1106::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  _vccstate = vccstate;
  _i2caddr = i2caddr;

  if((!buffer) && !(buffer = (uint8_t *)malloc(SH1106_LCDHEIGHT * SH1106_LCDWIDTH / 8)))
    return;
...
void Tiny_SH1106::invertDisplay(uint8_t i) {
  if (i) {
...
void Tiny_SH1106::SH1106_command(uint8_t c) { 
    // I2C
...
void Tiny_SH1106::SH1106_data(uint8_t c) {
    // I2C
...
void Tiny_SH1106::display(void) 
{
  ...
  for ( i = 0; i < height; i++) 
  {
        ...
        SH1106_command(2 & 0xf);//set lower column address
        SH1106_command(0x10 | (2 >> 4));//set higher column address
...
void Tiny_SH1106::clearDisplay(void) {
  memset(buffer, 0, (SH1106_LCDWIDTH*SH1106_LCDHEIGHT/8));
}


void Tiny_SH1106::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
  boolean bSwap = false;
  switch(rotation) { 
    case 0:
...
void Tiny_SH1106::drawFastHLineInternal(int16_t x, int16_t y, int16_t w, uint16_t color) {
...
void Tiny_SH1106::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
  bool bSwap = false;
  switch(rotation) { 
    case 0:
      break;
...
void Tiny_SH1106::drawFastVLineInternal(int16_t x, int16_t __y, int16_t __h, uint16_t color) {
